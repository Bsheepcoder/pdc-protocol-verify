# PDC Protocol — Parallel Data Channel Protocol

> 本文件定义 PDC 协议的完整规范，面向 AI Agent 和开发者。配合 /llms.txt 使用。

## 协议概述

PDC（Parallel Data Channel Protocol，平行数据通道协议）是一种面向静态博客/站点的内容交付协议。同一份内容在构建时以两种形态并行输出：人类通道（HTML）和机器通道（JSON/MD/TXT），运行时完全独立，零服务端逻辑。
...
| 层 | 职责 | 实现位置 | 必需 |
|----|------|---------|------|
| 数据通道层 | 8 个静态端点，AI 获取内容 | scripts/ai-api.js, scripts/ai-category.js | 是 |
...
PDC 协议兼容 llms.txt 标准（/llms.txt 格式），同时扩展了结构化索引、单篇内容、分类聚合、加密保护、内容注入等端点和规范。PDC 可视为 llms.txt 标志的超集实现。

## 参考实现

本站（https://bsheepcoder.github.io）是 PDC 协议的参考实现。
...
PDC 协议不仅是技术规范，也是一个**开放互链网络**。

### 如何加入

1. **实现 PDC 协议**：参照本文档，在你的 Hexo/静态站点上实现 `/llms.txt`、`/api/index.json`、`/api/posts/<slug>.md` 等端点
...
PDC 协议可通过薄适配器接入 MCP（Model Context Protocol）生态，让 Claude Desktop、Cursor 等 MCP 客户端直接访问站点内容。
